# agy 1.1.5 / 1.1.7 / 1.1.14 interop schema
...
The 1.1.14 extension retains the descriptor-verified `USER_INPUT` (14),
`RUN_COMMAND` (21), and `CHECKPOINT` (23) step identities and their `Step`
oneof fields (19, 28, and 30). Their payloads remain opaque; tool lifecycle
mapping relies only on the already parity-checked `metadata.tool_call` field.
...
1.1.7 closure. The selected 1.1.14 additions were checked against the exact
descriptors embedded in the installed 1.1.14 binary; provenance hashes and the
retained numeric contract live under `testdata/rpc/v1.1.14`. The full private
closure is caller-supplied only for validation:

```sh
AGY_DESCRIPTOR_SET=/path/to/descriptor-set.pb \
  GOCACHE=$PWD/.cache/go-build GOMODCACHE=$PWD/.cache/go-mod \
  go test ./agyclient/runtime/agy/interop/v115 -run TestParityAgainstSuppliedClosure
```
...
AGY_VERIFY_GENERATION=1 GOCACHE=$PWD/.cache/go-build GOMODCACHE=$PWD/.cache/go-mod \
  go test ./agyclient/runtime/agy/interop/v115 -run TestDeterministicGenerationClean
```
